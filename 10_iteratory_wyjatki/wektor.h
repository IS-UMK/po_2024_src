...
template <typename T>
Wektor<T>::Wektor(int r):rozmiar{r},liczba_elementow(0)
{ 
		if (r<0) throw Wyjatek("Rozmiar nie moze byc mienjszy od zera.");
		elementy =  new(std::nothrow) T[rozmiar];
		if (elementy == nullptr) throw BrakPamieci("Zabraklo pamieci podczas tworzenia wektora");
}
...
void Wektor<T>::Dodaj(const T& t)
{
	if (rozmiar == liczba_elementow)
	{
		...
		if (nowa == nullptr) throw BrakPamieci("Zabraklo pamieci podczas dodawania elementu");
...
{
	if(ind <0 || ind >=liczba_elementow) throw ZlyIndeksWektora(ind, "Wyjatek - zly indeks.");
	return elementy[ind]; 
}
